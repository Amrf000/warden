...
int32_t
CGxString::Initialize(float fontHeight, const C3Vector &position, float blockWidth, float blockHeight, CGxFont *face,
                      const char *text, EGxFontVJusts vertJust, EGxFontHJusts horzJust, float spacing, uint32_t flags,
                      const CImVector &color, float scale) {
...
void CGxString::InitializeTextLine(const char *currentText, uint32_t numBytes, CImVector &workingColor,
                                   const C3Vector &position, uint32_t *texturePagesUsedFlag, EMBEDDEDPARSEINFO &info) {
...
void CGxString::InitializeViewTranslation() {
    ...
    float v13 =
            ((this->m_spacing * (float) GetScreenPixelHeight()) + 0.9999499917030334f) / (float) GetScreenPixelHeight();

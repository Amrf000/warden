...
int32_t GxTexCreate(EGxTexTarget target, uint32_t width, uint32_t height, uint32_t depth, EGxTexFormat format,
                    EGxTexFormat dataFormat, CGxTexFlags flags, void *userArg, TEXTURE_CALLBACK *userFunc,
                    const char *name, CGxTex *&texId) {
    texId = nullptr;
...
CGxTex *TextureAllocGxTex(EGxTexTarget target, uint32_t width, uint32_t height, uint32_t depth, EGxTexFormat format,
                          CGxTexFlags flags, void *userArg, TEXTURE_CALLBACK *userFunc, EGxTexFormat dataFormat) {
    CGxTexParms gxTexParms;
...
void
GxuUpdateSingleColorTexture(EGxTexCommand cmd, uint32_t w, uint32_t h, uint32_t d, uint32_t mipLevel, void *userArg,
...
void
GetTextureFormats(PIXEL_FORMAT *pixFormat, EGxTexFormat *gxTexFormat, PIXEL_FORMAT preferredFormat, int32_t alphaBits) {
...
MipBits *MippedImgAllocA(uint32_t fourCC, uint32_t width, uint32_t height, const char *fileName, int32_t lineNumber) {
    ...
    MipBits *images = reinterpret_cast<MipBits *>(SMemAlloc(levelDataSize + sizeof(void *) * levelCount + 16, fileName,
                                                            lineNumber, 0));

    uintptr_t v10 = (reinterpret_cast<uintptr_t>(images) + sizeof(void *) * levelCount + 15) &
                    static_cast<uintptr_t>(-sizeof(void *));
...
void UpdateBlpTextureAsync(EGxTexCommand cmd, uint32_t w, uint32_t h, uint32_t d, uint32_t mipLevel, void *userArg,
                           uint32_t &texelStrideInBytes, const void *&texels) {
...
HTEXTURE
CreateTgaTexture(const char *fileName, const char *fileExt, int32_t a3, CGxTexFlags texFlags, CStatus *status) {
    // TODO
...
HTEXTURE
TextureCreate(uint32_t width, uint32_t height, EGxTexFormat format, EGxTexFormat dataFormat, CGxTexFlags texFlags,
              void *userArg, TEXTURE_CALLBACK *userFunc, const char *a8, int32_t a9) {
...
HTEXTURE TextureCreate(EGxTexTarget target, uint32_t width, uint32_t height, uint32_t depth, EGxTexFormat format,
                       EGxTexFormat dataFormat, CGxTexFlags texFlags, void *userArg, TEXTURE_CALLBACK *userFunc,
                       const char *a10, int32_t a11) {

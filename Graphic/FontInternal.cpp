...
uint32_t
InternalGetMaxCharsWithinWidth(CGxFont *face, const char *text, float height, float maxWidth, uint32_t lineBytes,
                               float *extent, uint32_t flags, float a8, float scale, uint32_t *bytesInString,
                               float *widthArray, float *widthArrayGuard) {
    STORM_ASSERT(face);
    STORM_ASSERT(text);
    STORM_ASSERT(extent);
...
void
InternalGetTextExtent(CGxFont *font, const char *text, uint32_t numBytes, float height, float *extent, uint32_t flags,
                      float a7, float a8) {
    STORM_ASSERT(font);
    STORM_ASSERT(text);
    STORM_ASSERT(extent);

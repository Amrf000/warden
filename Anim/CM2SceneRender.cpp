...
void CM2SceneRender::DrawBatch() {
    ...
    if (
        ...
        for (int32_t i = 0; i < this->m_curSkinSection->boneCount; i++) {
            auto &boneMatrix = this->m_curModel->m_boneMatrices[this->m_data->boneCombos[
                    this->m_curSkinSection->boneComboIndex + i]];
...
void CM2SceneRender::SetupTextures() {
    ...
    for (int32_t i = 0; i < 2; i++) {
        ...
        auto textureTransformIndex = this->m_data->textureTransformCombos[this->m_curBatch->textureTransformComboIndex +
                                                                          i];

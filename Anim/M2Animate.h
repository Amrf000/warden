...
void M2InterpolateCubicBezier(const M2SplineKey<C3Vector> &startKey, const M2SplineKey<C3Vector> &endKey, float ratio,
                              C3Vector &value) {
    // TODO
}

void M2InterpolateCubicBezier(const M2SplineKey<float> &startKey, const M2SplineKey<float> &endKey, float ratio,
                              float &value) {
    // TODO
}

void
M2InterpolateCubicHermite(const M2SplineKey<C3Vector> &startKey, const M2SplineKey<C3Vector> &endKey, float ratio,
                          C3Vector &value) {
    // TODO
}

void M2InterpolateCubicHermite(const M2SplineKey<float> &startKey, const M2SplineKey<float> &endKey, float ratio,
                               float &value) {
    // TODO
}
...
template<class T1, class T2>
void
M2AnimateSplineTrack(CM2Model *model, M2ModelBone *modelBone, const M2Track<T1> &track, M2ModelTrack<T2> &modelTrack,
...
template<class T1, class T2>
void M2AnimateTrack(CM2Model *model, M2ModelBone *modelBone, const M2Track<T1> &track, M2ModelTrack<T2> &modelTrack,
                    const T2 &defaultValue) {
    auto seqIndex = modelBone->sequence.uint4 < track.sequenceKeys.Count() ? modelBone->sequence.uint4 : 0;
    auto &seqKeys = track.sequenceKeys[seqIndex];

...
void CLayoutFrame::ResizePending() {
    ...
    for (CLayoutFrame *frame = LayoutFrame::s_resizePendingList.Head();
         frame && loading < 32; frame = LayoutFrame::s_resizePendingList.Link(frame)->Next()) {
...
void CLayoutFrame::AddToResizeList() {
    ...
    for (auto frame = LayoutFrame::s_resizePendingList.Head(); frame; frame = LayoutFrame::s_resizePendingList.Link(
            frame)->Next()) {
...
int32_t CLayoutFrame::IsResizeDependency(CLayoutFrame *dependentFrame) {
    ...
    for (int32_t i = 0; i < FRAMEPOINT_NUMPOINTS; i++) {
        ...
        if (framePoint->GetRelative() == dependentFrame ||
            framePoint->GetRelative()->IsResizeDependency(dependentFrame)) {
            return 1;
        }
...
void
CLayoutFrame::SetPoint(FRAMEPOINT point, CLayoutFrame *relative, FRAMEPOINT relativePoint, float offsetX, float offsetY,
                       int32_t doResize) {
    STORM_ASSERT(relative);
    STORM_ASSERT(relative != this);

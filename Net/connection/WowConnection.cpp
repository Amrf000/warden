...
WowConnection::SENDNODE::SENDNODE(void *data, int32_t size, uint8_t *buf, bool raw)
        : TSLinkedNode<WowConnection::SENDNODE>() {
    if (data) {
        this->data = buf;
    }
...
int32_t
WowConnection::InitOsNet(bool (*fcn)(const NETADDR *), void (*threadinit)(), int32_t numThreads, bool useEngine) {
    ...
    while (true) {
        ...
        if (this->m_readBytes >= headerSize) {
            if ((this->m_readBuffer[0] & 0x80) == 0) {
                ...
                if (this->m_readBytes >= headerSize) {
                    size = (this->m_readBuffer[2] |
                            ((this->m_readBuffer[1] | ((this->m_readBuffer[0] & 0x7F) << 8)) << 8)) + headerSize;
                }
            }
        }
        ...
        if (bytesToRead <= 0) {
            ...
            while (true) {
                bytesRead = recv(this->m_sock, reinterpret_cast<char *>(&this->m_readBuffer[this->m_readBytes]),
                                 bytesToRead, 0x0);
...
void WowConnection::SetEncryptionKey(const uint8_t *key, uint8_t keyLen, uint8_t a4, const uint8_t *seedData,
                                     uint8_t seedLen) {
    if (!seedData) {
        seedData = s_arc4seed;

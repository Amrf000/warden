...
void RealmConnection::PollNet() {
    ...
    for (auto node = RealmConnection::s_AllRealmConnections.Head(); node; node = RealmConnection::s_AllRealmConnections.Next(
            node)) {
...
int32_t RealmConnection::HandleAuthChallenge(AuthenticationChallenge *challenge) {
    ...
    SHA1_Update(&ctx, reinterpret_cast<const uint8_t *>(this->GetLoginData().m_account),
                SStrLen(this->GetLoginData().m_account));

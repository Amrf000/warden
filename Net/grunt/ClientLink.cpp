...
int32_t Grunt::ClientLink::CmdAuthLogonChallenge(CDataStore &msg) {
    ...
    if (this->m_srpClient.CalculateProof(largeSafePrime, largeSafePrimeLen, generator, generatorLen, salt, 32,
                                         serverPublicKey, 32, srpRandom)) {
        this->SetState(2);
        ...
        if (msg.m_read <= msg.m_size && msg.m_size - msg.m_read >= 2) {
            ...
            if (msg.m_read <= msg.m_size) {
                if (this->m_srpClient.VerifyServerProof(static_cast<uint8_t *>(serverProof), 20)) {
                    ...
                    this->SetState(6);
                    this->m_clientResponse->LogonResult(Grunt::GRUNT_RESULT_0, this->m_srpClient.sessionKey, 40,
                                                        logonFlags);
                }
...
void Grunt::ClientLink::WCConnected(WowConnection *conn, WowConnection *inbound, uint32_t timeStamp,

...
void CGlueMgr::DisplayLoginStatus() {
    // No change
    if (CGlueMgr::m_lastLoginState == CGlueMgr::m_loginState &&
...
int32_t CGlueMgr::HandleDisplaySizeChanged(const CSizeEvent &event) {
    ...
    auto glueParent = static_cast<CSimpleFrame *>(CScriptObject::GetScriptObjectByName("GlueParent",
                                                                                       CSimpleFrame::GetObjectType()));
    if (glueParent) {
...
void CGlueMgr::LoginServerLogin(const char *accountName, const char *password) {
    if (!CGlueMgr::m_scandllOkayToLogIn || !CGlueMgr::m_acceptedTOS || !CGlueMgr::m_acceptedEULA ||
    ...
    for (const char *src = accountName;
         *src && dest < CGlueMgr::m_accountName + sizeof(CGlueMgr::m_accountName); src++, dest++) {
        if (src[0] == '|' && src[1] == '|') {
            src++;
        }
...
void CGlueMgr::SetLoginStateAndResult(LOGIN_STATE state, LOGIN_RESULT result, const char *addrStr, const char *stateStr,
                                      const char *resultStr, uint8_t flags) {
    // TODO
